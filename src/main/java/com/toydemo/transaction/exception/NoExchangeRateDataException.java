package com.toydemo.transaction.exception;

import java.time.LocalDate;
public class NoExchangeRateDataException extends RuntimeException {

    public NoExchangeRateDataException(LocalDate startWindow, LocalDate transactionDate) {
        super("no exchange rate data exists between " + startWindow + " and " + transactionDate);
    }

    public NoExchangeRateDataException(String message, Throwable cause) {
        super(message, cause);
    }
}
